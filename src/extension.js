    ...
    insertToMap(clazz) {
        ...
        function customTypeMapping(prop, name = null, endFlag = ',\n') {
            ...
            switch (prop.type) {
                ...
                case 'Duration':
                    return `${name}${nullSafe}.inSeconds${endFlag}`;
                case 'Color':
        ...
        for (let p of props) {
            ...
            if (p.isEnum) {
                ...
                if (p.isMap || p.listType.isPrimitive) {
                    ...
                    method += `${p.name}${nullSafe}.map((x) => x.toMap()).toList(),\n`
                }
            } else {
            ...
            switch (prop.type) {
                ...
                default:
                    return `${!prop.isPrimitive ? prop.type + ".fromMap(" : ""
                        ...
                        }${value}${!prop.isPrimitive ? ")" : ""}${fromJSON
                            ? prop.isDouble
                                ? ".toDouble()"
                                : prop.isInt
                                    ? ".toInt()"
        ...
        for (let p of props) {
            ...
            if (p.isEnum) {
                ...
                if (p.isPrimitive) {
                    ...
                    const nullSafe = p.isNullable ? '?' : '';
                    method += `(${value}${nullSafe}.map((x) => ${customTypeMapping(p, 'x')},),${defaultValue})`;
